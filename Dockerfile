...
# 默认值指向本仓库缓存的稳定 URL；CI / 本地构建会按需用 build-arg 覆盖。
# 这样即使手工 `docker build .`，也优先使用当前仓库已缓存、可校验的 zip，减少对上游 release 的直接依赖。
# OPENPPP2_ZIP_SHA256 为该 zip 的 SHA256：非空时强制校验（CI/复现构建必校验），
# 留空则跳过校验并打印告警（仅用于无法预知 sha 的临时构建，例如直接追上游最新版本时）。
ARG OPENPPP2_ZIP_URL="https://github.com/lucifer988/openppp2-docker/releases/download/upstream-openppp2-cache/openppp2-linux-amd64-simd.zip"
