...
**核查**：`.github/workflows/build.yml` **确实存在**，CI 也按周运行过多次。你看到 404、且 Packages 区为空，根因不是"没有 workflow"，而是：

- 用 `GITHUB_TOKEN` 推送到 GHCR 的容器包**默认是私有（private）**，workflow 里**从未把它设为 public**，导致匿名 `docker pull` 失败、Packages 对外不可见；
- 原 workflow 只构建 **linux/amd64**，ARM64 主机即使能拉也用不了。

所以你的最终担心（镜像不公开 → 一键安装大概率在拉镜像阶段失败）**是成立的**。

**修复**：
- workflow 增加把 GHCR 包设为 public 的步骤（REST API，失败则提示一次性手动设置）；
- 改为多架构构建（amd64 + arm64），按架构下载对应 simd 包；
- 安装脚本新增 `ensure_image`：`docker pull` 失败时自动用 `Dockerfile` 在本机构建（识别 amd64/aarch64），**即使 GHCR 不可用也能装成功**。
...
**修复**：`openppp2-update.sh` 改为：快照旧镜像 ID + 备份配置 → 更新 → 健康检查（Running + ppp 进程）→ 失败时把旧镜像重新 `tag` 回去、恢复配置、再次 `up`，实现**健康检查门控的真实回滚**（可用 `AUTO_ROLLBACK=no` 关闭）。
...
**修复**：
- `health_check_one` 增强为：Running → （若有）HEALTHCHECK healthy → 容器内 ppp 进程存在 → 角色探测（server 查端口 LISTEN；client 查 TUN 设备 + 经 HTTP 代理真实出网）；
...
- 新增文件：`.dockerignore`、`CHANGES.md`，以及多架构 + public 的 `build.yml`
