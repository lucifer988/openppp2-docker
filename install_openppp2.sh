#!/usr/bin/env bash
# =============================================================================
#  openppp2-docker — One-click installer
...
on_err() {
  local ec=$? line=${1:-?}
  err "脚本第 ${line} 行异常退出（exit=${ec}）。如需协助请到 ${REPO_URL}/issues 反馈。"
  exit "$ec"
}
...
# 轻量级 compose 探测：只检测当前已安装的，不会主动安装。供菜单中“查看/新增/删除/备份/恢复”等
# 不经过 ensure_stack_ready 的操作使用，避免出现 “compose 命令未初始化” 的错误。
...
do_install() {
  ...
  local main_svc="openppp2"
  if [[ "$ROLE" == "2" ]] && [[ -f "$MAIN_SERVICE_FILE" ]]; then
    main_svc="$(cat "$MAIN_SERVICE_FILE")"
  fi
  sleep 2
  health_check_one "$main_svc" || warn "健康检查未通过，请查看日志。"

  setup_systemd_weekly_update

  if (( proxy_cleanup_deferred )); then
    echo
    warn "Docker 代理配置文件已删除，但未重启 Docker，避免顺手停掉容器。"
    warn "如要让 Docker 立即丢弃代理：systemctl restart docker（openppp2 会被自动拉起）"
  fi
  ...
  echo "再次进入菜单：bash <(curl -fsSL ${RAW_BASE}/install_openppp2.sh)"
}
...
do_uninstall() {
  ...
  if has_systemd; then
    systemctl disable --now openppp2-update.timer 2>/dev/null || true
    systemctl disable --now openppp2-update.service 2>/dev/null || true
  fi
  ...
  if need_cmd docker; then
    local cs
    cs="$(docker ps -a --filter name=openppp2 --format '{{.Names}}' 2>/dev/null || true)"
    if [[ -n "$cs" ]]; then
      info "停止并删除 openppp2 容器..."
      while read -r c; do [[ -n "$c" ]] && docker rm -f "$c" >/dev/null 2>&1 || true; done <<<"$cs"
    fi
  fi

  if [[ -d "$APP_DIR" ]] && need_cmd docker; then
    detect_compose >/dev/null 2>&1 || true
    [[ -n "$COMPOSE_KIND" ]] && (cd "$APP_DIR" && compose down --remove-orphans >/dev/null 2>&1) || true
  fi
  ...
  if [[ "$KEEP_BACKUP" == "yes" && -d "$BACKUP_DIR" ]]; then
    local tmp_backup="/tmp/openppp2-backups-$(date +%Y%m%d_%H%M%S)"
    mv "$BACKUP_DIR" "$tmp_backup"
    rm -rf "$APP_DIR"
    mkdir -p "$APP_DIR"
  ...
  rm -f /etc/sysctl.d/99-openppp2.conf
  sysctl --system >/dev/null 2>&1 || true
  remove_docker_proxy
...
print_banner() {
  cat <<EOF
==============================
  openppp2 一键部署脚本  v${SCRIPT_VERSION}
  仓库: ${REPO_URL}
==============================
请选择操作：
  1) 安装 openppp2
