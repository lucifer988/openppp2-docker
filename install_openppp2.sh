...
do_install() {
  ...
  if [[ "$ROLE" == "1" ]]; then
    ...
    validate_cfg_name "$APP_CFG_NAME"
    ...
    validate_service_name "$MAIN_SERVICE_NAME"
  ...
  # ★ 可复现修复（生产 #2 / 自动更新 #2）：把 compose 固定到不可变 digest，
  #   并记录浮动发现通道（.image_channel）供后续自动更新发现新版本。
  pin_compose_to_digest "$IMAGE"

  # ★ 持久化本次选定的可调参数，供自动更新/脚本自更新无人值守时复用。
  persist_env_file
  ...
  if [[ "$ROLE" == "2" ]]; then
    ...
    health_check_one "$main_svc" "client" "${lan}|${HTTP_PORT}|${SOCKS_PORT}|${S_USER}|${S_PASS}" ||
...
do_uninstall() {
  ...
  validate_app_dir # 防止 APP_DIR 被环境变量误设后 rm -rf 误删系统目录
  ...
  rm -f /etc/systemd/system/openppp2-update.timer \
    ...
    /etc/systemd/system/openppp2-notify@.service \
    /usr/local/bin/openppp2-notify.sh \
    /usr/local/bin/openppp2-self-update.sh \
...
# 非交互维护入口（供脚本自更新 openppp2-self-update.sh 调用）：
#   仅用当前（已更新的）代码重新生成 systemd 助手脚本与单元，不触碰配置与容器。
#   config.sh 已在上方 source 时读取 ${APP_DIR}/.env，故用户既有选项（通知/回滚/资源等）得以保留。
if [[ "${1:-}" == "--regen-systemd" ]]; then
  check_env_supported
  validate_app_dir
  detect_compose >/dev/null 2>&1 || true
  setup_systemd_weekly_update
