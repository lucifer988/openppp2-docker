...
build_image_local() {
  ...
  case "$(uname -m)" in
    ...
    *) die "未知 CPU 架构 $(uname -m)，无法本地构建。" ;;
  esac
  zipurl="https://github.com/${UPSTREAM_REPO}/releases/download/${tag}/openppp2-linux-${arch}-simd.zip"

  info "本地构建镜像 ${image}（上游 ${tag} / ${arch}）..."
...
#   role  : server | client
#   probe : server -> 监听端口（默认 20000）；client -> http 代理 "IP:PORT"
# 返回 0 仅当：容器 Running（且若有 HEALTHCHECK 则为 healthy）+ 角色功能探测通过
health_check_one() {
  ...
  case "$role" in
    server)
      ...
      if docker exec "$name" sh -c "command -v ss >/dev/null 2>&1"; then
        if ! docker exec "$name" sh -c "ss -ltn 2>/dev/null | grep -q ':${port}'"; then
          warn "服务端未在端口 ${port} 上监听（TCP）。"
          return 1
        fi
      fi
      info "服务端健康检查通过（容器运行 + ppp 进程 + 监听 ${port}）。"
      ;;
    client)
      # TUN 设备
      ...
      # 通过 HTTP 代理做一次真实出网探测（best-effort）
      if [[ -n "$probe" ]] && need_cmd curl; then
        if curl -fsS -x "http://${probe}" -m 8 -o /dev/null \
             https://www.gstatic.com/generate_204 2>/dev/null; then
          info "客户端健康检查通过（容器运行 + TUN + 代理出网 ${probe} 正常）。"
        else
          warn "客户端容器与 TUN 正常，但经 HTTP 代理 ${probe} 出网探测失败（隧道可能尚未建立，可稍后重试）。"
          return 1
        fi
      else
        info "客户端健康检查通过（容器运行 + ppp 进程 + TUN）。"
      fi
      ;;
    *)
