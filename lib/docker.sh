...
# === health_check_one ===
# 三层判断：
#   1) 容器存在且在 docker ps 列表里（最基础）
#   2) 如果镜像里带了 HEALTHCHECK（v2.2 Dockerfile 已加），看 .State.Health.Status
#      - starting : 还在 start-period 内，等等再说
#      - healthy  : OK
#      - unhealthy: 失败
#      - 镜像没 HEALTHCHECK 时返回 no-healthcheck，回退到日志关键字判断
#   3) 扫日志：如果日志里出现 io_uring 拒绝，说明 seccomp 没生效
health_check_one() {
  local svc="$1"
  local ok=0
  ...
  # 优先使用镜像自带 HEALTHCHECK 的结果（v2.2 镜像内置）
  local hstatus
  hstatus="$(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}no-healthcheck{{end}}' "$svc" 2>/dev/null || echo unknown)"

  case "$hstatus" in
    healthy)
      info "${svc} 健康检查通过（HEALTHCHECK=healthy）"
      ;;
    starting)
      # 给 start-period 多一点宽限：再等 10 秒抽查一次
      sleep 10
      hstatus="$(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}no-healthcheck{{end}}' "$svc" 2>/dev/null || echo unknown)"
      if [[ "$hstatus" == "unhealthy" ]]; then
        warn "${svc} 健康检查失败（HEALTHCHECK=unhealthy）"
        ok=1
      fi
      ;;
    unhealthy)
    ...
    no-healthcheck|unknown|*)
      # 老镜像没 HEALTHCHECK，沿用以前的行为，只看日志关键字
      :
      ;;
  esac
