...
# 限时执行：有 timeout 命令则限时（超时发 TERM，再宽限 20s 后 KILL），否则直接执行。
# 用于包住"会从远端下载包"的安装命令，网络差时能及时放弃、不至于整脚本卡死。
_run_with_timeout() {
  local secs="${1:-0}"
  shift
  if [[ "$secs" =~ ^[0-9]+$ ]] && ((secs > 0)) && need_cmd timeout; then
    timeout --signal=TERM --kill-after=20 "$secs" "$@"
  else
    "$@"
  fi
}

# 快速探测 download.docker.com 是否可用（5s 连接 / 10s 总超时）。可用返回 0。
# 国内网络通常在此超时/失败，从而触发自动改用国内镜像源。
_download_docker_reachable() {
  need_cmd curl || return 1
  curl -fsS --connect-timeout 5 --max-time 10 -o /dev/null \
    "https://download.docker.com/linux/ubuntu/gpg" 2>/dev/null
}

# 用本地的 get-docker.sh 按给定镜像参数执行一次；装上 docker 返回 0。
#   script : 已下载到本地的 get-docker.sh 路径（复用，避免每个镜像重复下载）
#   mirror : "" 表示官方源；"Aliyun" / "AzureChinaCloud" 表示对应国内镜像
_run_get_docker_script() {
  local script="$1" mirror="$2" label="$3"
  local -a args=()
  [[ -n "$mirror" ]] && args=(--mirror "$mirror")
  info "通过官方脚本安装 Docker（${label}）..."
  if _run_with_timeout "$DOCKER_INSTALL_TIMEOUT" sh "$script" "${args[@]}"; then
    need_cmd docker && {
      info "Docker 安装成功（${label}）。"
      return 0
    }
  fi
  warn "本次安装失败或超时（${label}）。"
  return 1
}

# 多渠道安装 Docker，任一成功即返回 0
install_docker() {
  # 渠道 1：官方便捷脚本（get.docker.com）。
  #   国内 download.docker.com 慢/不通会卡在"安装 Docker"这一步，故：
  #     1) 先把脚本下载到本地（避免管道断流执行残缺脚本，也便于复用给多个镜像源）；
  #     2) 支持 --mirror（Aliyun / AzureChinaCloud），把重头的安装包从国内镜像拉取；
  #     3) DOCKER_INSTALL_MIRROR=auto 时自动探测官方源是否可达，不可达就优先国内镜像；
  #     4) 每次尝试限时（DOCKER_INSTALL_TIMEOUT），超时即换下一镜像/渠道，绝不无限卡住。
  if need_cmd curl; then
    ...
    if curl -fsSL --retry 3 --retry-delay 3 --connect-timeout 15 https://get.docker.com -o "$tmp_sh" &&
      [[ -s "$tmp_sh" ]]; then
      # 依据 DOCKER_INSTALL_MIRROR 决定镜像尝试顺序。
      #   - 强制单一镜像（Aliyun/AzureChinaCloud/none）：只试该一种，尊重用户意图、不浪费时间；
      #   - auto：探测官方源可达性，决定"官方优先回退国内"还是"国内优先回退官方"。
      local -a order=()
      case "$DOCKER_INSTALL_MIRROR" in
        none) order=("") ;;
        Aliyun) order=("Aliyun") ;;
        AzureChinaCloud) order=("AzureChinaCloud") ;;
        auto | *)
          if _download_docker_reachable; then
            info "检测到 download.docker.com 可正常访问，优先使用官方源。"
            order=("" "Aliyun" "AzureChinaCloud")
          else
            warn "download.docker.com 访问受限（疑似国内网络），优先使用国内镜像源安装 Docker。"
            order=("Aliyun" "AzureChinaCloud" "")
          fi
          ;;
      esac
      ...
      for m in "${order[@]}"; do
        if [[ -n "$m" ]]; then
          label="get.docker.com 镜像源：${m}"
        else
          label="get.docker.com 官方源"
        fi
        if _run_get_docker_script "$tmp_sh" "$m" "$label"; then
          rm -f "$tmp_sh"
          return 0
        fi
      done
    else
      warn "无法下载 get.docker.com 安装脚本（网络受限）。"
    fi
    rm -f "$tmp_sh"
    warn "官方脚本各镜像源均失败/超时，改用发行版软件源..."
  fi
