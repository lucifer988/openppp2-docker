#!/usr/bin/env bash
# lib/seccomp.sh — 自定义 seccomp profile 生成
#
# 在 Docker 默认 seccomp 基础上放开 openppp2 需要的 io_uring 相关系统调用。
# 这个 profile 由 compose.sh 通过 security_opt 引用。

SCRIPT_DIR_SECCOMP="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
[[ -f "${SCRIPT_DIR_SECCOMP}/config.sh" ]] && source "${SCRIPT_DIR_SECCOMP}/config.sh"

# === generate_seccomp_profile <out_path> ===
# 生成放开 io_uring 的最小化 seccomp profile。
# 基于 Docker 默认 seccomp，额外允许：
#   io_uring_setup / io_uring_register / io_uring_enter
generate_seccomp_profile() {
  local out="$1"
  if [[ -z "$out" ]]; then
    die "generate_seccomp_profile: 缺少输出路径参数"
  fi
  mkdir -p "$(dirname "$out")"
  info "生成 seccomp profile：$out"
  ...
  "syscalls": [
    {
      "names": [
        ...
        "futex", "futex_time64", "futex_waitv", "futimesat", "getcpu", "getcwd",
        ...
        "ipc", "kill", "landlock_add_rule", "landlock_create_ruleset", "landlock_restrict_self",
        "lchown", "lchown32", "lgetxattr", "link", "linkat", "listen", "listxattr",
        "llistxattr", "_llseek", "lremovexattr", "lseek", "lsetxattr", "lstat", "lstat64",
        "madvise", "membarrier", "memfd_create", "memfd_secret", "mincore", "mkdir",
        "mkdirat", "mknod", "mknodat", "mlock", "mlock2", "mlockall", "mmap", "mmap2",
        "mprotect", "mq_getsetattr", "mq_notify", "mq_open", "mq_timedreceive",
        "mq_timedreceive_time64", "mq_timedsend", "mq_timedsend_time64", "mq_unlink",
        "mremap", "msgctl", "msgget", "msgrcv", "msgsnd", "msync", "munlock",
        "munlockall", "munmap", "name_to_handle_at", "nanosleep", "newfstatat", "_newselect",
        "open", "openat", "openat2", "pause", "pidfd_open", "pidfd_send_signal",
        "pipe", "pipe2", "pkey_alloc", "pkey_free", "pkey_mprotect", "poll", "ppoll",
        "ppoll_time64", "prctl", "pread64", "preadv", "preadv2", "prlimit64", "process_mrelease",
        "pselect6", "pselect6_time64", "pwrite64", "pwritev", "pwritev2", "read",
        "readahead", "readlink", "readlinkat", "readv", "recv", "recvfrom", "recvmmsg",
        "recvmmsg_time64", "recvmsg", "remap_file_pages", "removexattr", "rename",
        "renameat", "renameat2", "restart_syscall", "rmdir", "rseq", "rt_sigaction",
        "rt_sigpending", "rt_sigprocmask", "rt_sigqueueinfo", "rt_sigreturn",
        "rt_sigsuspend", "rt_sigtimedwait", "rt_sigtimedwait_time64", "rt_tgsigqueueinfo",
        ...
        "shmget", "shutdown", "sigaltstack", "signalfd", "signalfd4", "sigprocmask",
        ...
        "truncate64", "ugetrlimit", "umask", "uname", "unlink", "unlinkat", "utime",
        "utimensat", "utimensat_time64", "utimes", "vfork", "vmsplice", "wait4",
        "waitid", "waitpid", "write", "writev"
      ],
      "action": "SCMP_ACT_ALLOW"
    },
    {
      "names": ["bpf", "clone", "clone3", "fanotify_init", "fsconfig", "fsmount", "fsopen", "fspick", "lookup_dcookie", "mount", "mount_setattr", "move_mount", "open_tree", "perf_event_open", "personality", "ptrace", "quotactl", "quotactl_fd", "reboot", "setdomainname", "sethostname", "setns", "settimeofday", "stime", "swapoff", "swapon", "syslog", "umount", "umount2", "unshare", "uselib", "userfaultfd", "ustat", "vhangup"],
      "action": "SCMP_ACT_ALLOW"
    },
    {
      "names": ["socket"],
