...
curl_retry() {
  local curl_args=(-fL --retry 5 --retry-delay 2 --connect-timeout 10 --max-time 180)

  # curl 7.71+ 才支持 --retry-all-errors；老系统（例如部分 Ubuntu 20.04）
  # 没有该参数时直接跳过，避免一键安装因为 curl 版本过旧而失败。
